...
BEARER_AUTH = {"Authorization": f"Bearer {ACCESS_TOKEN}"}
...
def get_all_album_ids():
    ...
    bts_albums_json = requests.get(bts_album_endpoint, headers=BEARER_AUTH).json()
...
def get_all_tracks():
    ...
    for album_id in album_ids:
        tracks_endpoint = (
            f"https://api.spotify.com/v1/albums/{album_id}/tracks?limit=40"
        )
        bts_tracks_json = requests.get(tracks_endpoint, headers=BEARER_AUTH).json()
        all_album_tracks.append(get_ids(bts_tracks_json))
        all_track_names_and_ids.append(get_ids(bts_tracks_json, with_names=True))
...
def write_audio_feature_csv_files():
    ...
    for index, ids in enumerate(all_tracks_split_arr):
        audio_features_endpoint = (
            f'https://api.spotify.com/v1/audio-features?ids={",".join(ids)}'
        )
        audio_features_json = requests.get(
            audio_features_endpoint, headers=BEARER_AUTH
        ).json()
        csv_columns = audio_features_json["audio_features"][0].keys()

        if index == 0:
            with open(
                "data/bts_songs_spotify_audio_features.csv", "w", encoding="utf-8"
            ) as new_csv:
        ...
        try:
            with open(
                "data/bts_songs_spotify_audio_features.csv", "a", encoding="utf-8"
            ) as csvfile:
    ...
    name_and_id_df = pd.DataFrame(track_names_and_ids, columns=["spotify_id", "name"])
    name_and_id_df = remove_duplicate_songs(name_and_id_df)
    bts_audio_features = pd.read_csv("data/bts_songs_spotify_audio_features.csv")
    ...
    final_df.to_csv(
        "data/bts-songs-names-and-features-spotify.csv", index=False, encoding="utf-8"
    )
